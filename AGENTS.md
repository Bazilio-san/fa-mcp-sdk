...
## End-to-End SDK Verification (Skill /verify-sdk)

Checks that a project generated by the **published** `fa-mcp-sdk` package really builds and runs. The runner
`.claude/skills/verify-sdk/scripts/run-verification.js` installs the CLI globally from npm, deletes and
regenerates a throw-away project from the skill's own `config.local.yaml` (kept out of version control by the
`*.local.*` rule — start from `config.example.yaml`), then drives it through `npm install`, a clean `yarn ci`,
`yarn lint`, `yarn format`, `yarn cb` and a real `yarn start` probed on `/health`.

```bash
node .claude/skills/verify-sdk/scripts/run-verification.js               # the whole scenario
node .claude/skills/verify-sdk/scripts/run-verification.js --only lint,format --from build --keep-running
```

Per-step logs go to a `verify-sdk-logs` directory next to the test project. Any failure is a defect in this
repository (`src/template/**`, `cli-template/**`, `bin/fa-mcp.js`, `src/core/**`, `package.json`), never in the
generated copy, which is deleted on the next run. Because the scenario installs from the npm registry, a fix
only counts once it is committed, pushed and published — see the repair loop in the skill.

Triggers: user asks to verify or smoke-test that the SDK works end to end, or to run the generation scenario.
