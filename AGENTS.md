...
Checks that a project generated by the **published** `fa-mcp-sdk` package really builds and runs. Invoke the
`/verify-sdk` skill — the steps, the runner and its flags, the setup and the repair loop all live there.
