...
├── tools/                    # one tool = one file (see "Tool organization" below)
│   ├── <tool-name>.ts        # a self-contained tool: definition + handler together in one file
│   ├── tool.ts               # ITemplateTool interface (shared helper, not a tool)
│   ├── tools.ts              # registry — lists the tool files, derives the Tool[] array
│   └── handle-tool-call.ts   # dispatcher — routes tools/call to each tool's handler by name
├── prompts/
...
replaced by `-` and the `.ts` extension — a tool whose `name` is `get_currency_rate` lives in
`src/tools/get-currency-rate.ts`. **Everything that belongs to the tool lives in that one file**: its
`inputSchema` (and any `outputSchema` / `execution`), its `title` and `description`, its `handler`, and any
helpers or UI markup it alone uses. Never split one tool into a separate schema file and a separate handler
file, and never keep all tools inline in `tools.ts` as one big array with a `switch (name)` dispatcher — that
inline shape is exactly the anti-pattern this rule removes.

Each tool file exports a self-contained `ITemplateTool` (`{ definition, handler }`, interface in
`src/tools/tool.ts`). Two thin files aggregate them and must stay thin:

- `src/tools/tools.ts` — the **registry**: it imports every tool file, lists them, and derives the `Tool[]`
  array advertised in `tools/list`.
- `src/tools/handle-tool-call.ts` — the **dispatcher**: it builds a name → handler map from the registry and
  routes each `tools/call` to the matching handler, applying the cross-cutting concerns (logging, the
  `DEBUG=mcp:tool` hook, the unknown-tool guard).

To add a tool: create `src/tools/<your-tool>.ts` exporting an `ITemplateTool`, then add its export to the
list in `src/tools/tools.ts`. The dispatcher picks it up automatically — no `switch` to edit.

A helper or schema fragment **shared by several tools is NOT a tool**, so it lives in `src/tools/` under a
descriptive name that is **not** any tool's `name` (for example `src/tools/widget-document.ts` for the shared
widget markup, or `src/tools/tags-schema.ts` for a schema fragment several tools reuse). A file whose name
matches a tool's `name` is the tool itself, never a helper.
...
Create one file per tool in `src/tools/` (`<tool-name>.ts`, `_` → `-`) — see "Tool organization" above and
`02-1-tools-and-api.md`.

#### Step 2: Write Code

- `src/tools/<tool-name>.ts` — one file per tool: its definition and handler together (see "Tool organization")
- `src/tools/tools.ts` — register each new tool file in the list
...
Use `gpt-5.4` as the model in Agent Tester — it provides the best balance of tool-calling accuracy and reasoning for MCP testing. Send real messages through the Headless API, observe how the LLM uses your tools, and refine:
...
A prompt-plan is an implementation plan written as a prompt for an executor, human or agent. Its format lives in
exactly one file — **`.claude/skills/_shared/prompt-plan-format.md`** — and that file is the authority. Read it
before writing a plan; do not reconstruct the rules from memory and do not copy them into other documents.

What it prescribes, so you know when it applies: a plain-language opening block written for a non-programmer with
nothing above it, a note to the executor as the second section, a checklist of implementation stages whose boxes
the executor ticks in the plan itself as work genuinely lands, the documentation update as the final stage, and
target-state wording throughout — decisions stated as decisions, never as a transition from an old state.

The same file is used by the `create-mcp-wizard` and `feature-prompt-generator` skills, so a change made there
