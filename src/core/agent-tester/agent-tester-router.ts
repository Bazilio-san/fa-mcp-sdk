...
export function createAgentTesterRouter(
  ...
  router.get('/api/auth-token', async (req, res): Promise<void> => {
    ...
    // Only issue JWT if we actually have a way to sign one — checks mode + key availability.
  ...
  // API: Refresh JWT auth token. Returns a freshly-signed JWT for the configured mode.
  // - legacyAesCtr: signed with HS256 + encryptKey
  // - embedded:     signed with autogenerated ES256/RS256 private key
  // - localKey:     signed with privateKeyPath (when configured)
  // - remoteJwks:   501 — clients must refresh against the external IdP, not us
  router.post('/api/auth-token/refresh', async (req, res): Promise<void> => {
    const auth = appConfig.webServer?.auth;
    if (!auth?.enabled) {
      res.status(400).json({ error: 'auth_disabled', error_description: 'Auth is not enabled on this server' });
      return;
    }
    if (!canLocallyIssueJwt()) {
      res.status(501).json({
