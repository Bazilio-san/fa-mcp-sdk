...
const revokedTokensSet: Set<string> = new Set((Array.isArray(revoked.jwtTokens) ? revoked.jwtTokens : []).map((e: any) => trim(e?.token)).filter(Boolean));

const revokedUsersSet: Set<string> = new Set((Array.isArray(revoked.users) ? revoked.users : []).map((u: any) => trim(u).toLowerCase()).filter(Boolean));
