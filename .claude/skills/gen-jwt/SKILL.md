...
- Ask: "Link to application? (enter application ID or Enter to skip)"
- If user says "no", "skip", "-", or presses Enter — omit the `request` param.

If the user did NOT mention IP restrictions:
- Ask: "Restrict by IP? (enter IP/CIDR separated by commas or Enter to skip)"
...
User: "jwt for svc-account, my-mcp service, for 8 days"
```bash
...
- If the user provides ttl in natural language ("1 year", "30 days", "for a year"), convert it to the CLI format: `1y`, `30d`, etc.
